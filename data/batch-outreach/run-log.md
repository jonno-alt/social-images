# Batch Outreach Run Log

## Run: 2026-03-24 (Automated — Research Phase Only)

- **Segment:** Australian Independent Schools (General)
- **Targets researched:** 1 valid (Stuartholme School)
- **Targets skipped:** 40+ (all had prior Gmail contact)
- **Status:** Researched - pending Gemini
- **Notes:** First automated run. No next-batch.txt, outreach-tracker.xlsx, or run-log.md existed — bootstrapped from scratch. The Australian Independent Schools segment is effectively exhausted. Jonno has contacted virtually every independent school across QLD, NSW, VIC, SA, TAS, NT, and ACT. Only Stuartholme School (stuartholme.com) had zero prior contact. Recommend Jonno creates next-batch.txt to specify new segments (e.g., international schools, corporate sector, conference producers, speaker bureaus).
- **Output file:** research-ready-2026-03-24.md

## Run: 2026-03-24 Run 4 (Automated — Research Phase Only)

- **Segment:** Professional and industry associations in Sydney
- **Targets researched:** 6 valid
- **Targets skipped:** 13 (small volunteer-run precinct chambers, 1 already contacted)
- **Status:** Researched - pending Gemini
- **Notes:** City-level Sydney associations. Backfilled to tracker on 2026-03-25. Targets: Committee for Sydney, BESydney, CBD Sydney Chamber, Parramatta Chamber, Western Sydney Leadership Dialogue, Canterbury Bankstown Chamber.
- **Output file:** research-ready-2026-03-24-run4.md

## Run: 2026-03-25 Run 5 (Automated — Research Phase Only)

- **Segment:** Professional and industry associations in Melbourne
- **Targets researched:** 6 valid
- **Targets skipped:** 10 (VCCI/MCC/CfM merged and heavily contacted, CEDA contacted 2021, Associations Forum contacted 2021+2026, AHA Vic active relationship, FinTech Australia Sydney-based, MBN volunteer-run, APDA no named leader, Melbourne Leadership Group is a consulting firm)
- **Status:** Researched - pending Gemini
- **Notes:** Melbourne city-level associations. VCCI dominates Melbourne's business association landscape (merged with Melbourne Chamber and Committee for Melbourne). Focused on independent industry-specific bodies. 3 FALLBACK emails (Melbourne Royal, VTA, VACC) — Gemini can verify CEO direct emails. 2 CONFIRMED (Manufacturing Australia, YRBA). 1 HIGH (MCB inferred from firstname@ pattern).
- **Output file:** research-ready-2026-03-25.md

## Run: 2026-03-25 Run 6 (Automated — Research Phase Only)

- **Segment:** Professional and industry associations in Brisbane
- **Targets researched:** 6 valid
- **Targets skipped:** 12 (Business Chamber Qld/CCIQ heavily contacted, QTIC draft Mar 2026, QHA active relationship, QTA draft Mar 2026, QFF contacted 2023-2024, QRC draft Mar 2026, QLS contacted Mar 2026, NRA contacted Oct-Nov 2024, Property Council contacted Mar 2026, AICD contacted 2024-2026, IAQ contacted 2021, Brisbane North Chamber volunteer-run)
- **Status:** Researched - pending Gemini
- **Notes:** Brisbane city-level associations. The Brisbane association landscape is heavily penetrated — 12 major associations skipped due to prior contact, many with recent Mar 2026 drafts from previous batch outreach runs. Only 1 CONFIRMED email (CCF Qld from official website), 5 FALLBACK (generic inboxes). Gemini should verify CEO direct emails for all FALLBACK targets. Targets: Committee for Brisbane, UDIA Queensland, Master Builders Queensland, QMCA, CCF Queensland, QTLC. Master Builders CEO Paul Bidwell has announced resignation effective May 2026. Brisbane associations segment likely near exhaustion — recommend moving to Perth or Brisbane conference producers next.
- **Output file:** research-ready-2026-03-25-run6.md

## Run: 2026-03-25 Run 7 (Automated — Research Phase Only)

- **Segment:** Professional and industry associations in Australia (national)
- **Targets researched:** 6 valid
- **Targets skipped:** 20 (AuSAE contacted 2024-2026, Governance Institute draft Mar 2026, TTF draft Mar 2026, AIM sent Mar 2026, AICD drafts Mar 2026, Ai Group sent Nov 2024, Minerals Council sent 2022, ACCI sent 2024, AHRI sent Mar 2026, Property Council sent Mar 2026, CPA Australia sent Mar 2026, NFF draft Mar 2026, ICA sent Mar 2026, ALC draft Mar 2026, Consult Australia sent Mar 2026, ARA drafts Mar 2026, RCSA draft Mar 2026, AIIA sent Mar 2026, ABA sent 2024, HIA sent Mar 2026)
- **Status:** Researched - pending Gemini
- **Notes:** National-level peak bodies. This segment is heavily penetrated — 20 major national associations already contacted, mostly in the Mar 2026 batch outreach wave. All 6 new targets have FALLBACK generic inbox emails only (comms@, info@, afgc@, contact@). Gemini should attempt to discover CEO direct emails for all targets. Targets: BCA (Bran Black), FSC (Blake Briggs), AFGC (Colm Maguire), FCA (Jay Westbury), COSBOA (Skye Cappuccio), IPA (Adrian Dwyer). National associations segment near exhaustion — recommend moving to NZ associations or Australian state-level segments next.
- **Output file:** research-ready-2026-03-25-run7.md

## Run: 2026-03-25 Run 8 (Automated — Research Phase Only)

- **Segment:** Professional and industry associations in New Zealand
- **Targets researched:** 6 valid
- **Targets skipped:** 5 (BusinessNZ sent Mar 2026, IoD NZ multiple emails 2024-2026, Infrastructure NZ sent Mar 2026, Property Council NZ active relationship 2025-2026, Civil Contractors NZ sent Mar 2026)
- **Status:** Researched - pending Gemini
- **Notes:** NZ national-level associations. First NZ segment processed. 2 CONFIRMED emails (TIA Rebecca Ingram from official team page, Retail NZ Carolyn Young from official team page). 2 HIGH (Hospitality NZ role-based ceo@ email from official site, NZGBC Andrew Eagles inferred from firstname.lastname@ pattern of colleagues). 2 FALLBACK generic inboxes (Cooperative Business NZ info@, FGC admin@) — Gemini should verify CEO direct emails for these. NZ association landscape is less penetrated than AU — good pipeline for further waves. Targets: TIA (Rebecca Ingram), Hospitality NZ (Kristy Phillips), Retail NZ (Carolyn Young), NZGBC (Andrew Eagles), Cooperative Business NZ (Saya Wahrlich), FGC (Raewyn Bleakley).
- **Output file:** research-ready-2026-03-25-run8.md
